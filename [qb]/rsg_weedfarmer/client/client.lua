...
function DestroyPlant()
    ...
    if not hasDone then
        ...
        local ped = GetPlayerPed(-1)
        isDoingAction = true
        TriggerServerEvent('rsg_weedfarmer:plantHasBeenHarvested', plant.id)
        ...
        QBCore.Functions.Progressbar("destroying_weed", "Destroying...", 5000, false, false, {
            ...
            TriggerServerEvent('rsg_weedfarmer:destroyPlant', plant.id)
            isDoingAction = false
            canHarvest = true
...
function HarvestWeedPlant()
    ...
    if not hasDone then
        ...
        QBCore.Functions.Progressbar("harvesting_weed", "Harvesting...", 5000, false, false, {
            ...
            TriggerServerEvent('rsg_weedfarmer:harvestWeed', plant.id)
            isDoingAction = false
            canHarvest = true
            FreezeEntityPosition(ped, false)
            ClearPedTasksImmediately(ped)
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true) < 1.3 and not isDoingAction and not v.beingHarvested and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                    if PlayerJob.name == 'police' then
                        ...
                        if v.growth < 100 then
                            ...
                            if IsControlJustReleased(0, Keys["G"]) then
                                if v.id == plant.id then
                                    TriggerEvent('rsg_weedfarmer:client:waterPlant')
                                end
                            elseif IsControlJustReleased(0, Keys["H"]) then
                                if v.id == plant.id then
                                    TriggerEvent('rsg_weedfarmer:client:feedPlant')
                                end
                            end
                        else
                ...
                if IsControlJustReleased(0, Keys["G"]) then
                    ...
                    QBCore.Functions.Progressbar("searching_seed", "Searching...", 10000, false, false, {
						...
						if math.random(1, 10) == 7 then
							TriggerServerEvent('rsg_weedfarmer:server:giveSeed')
						else
							QBCore.Functions.Notify("Nothing found this time!", "error")
						end
                    end)
                end
            else
...
RegisterNetEvent('rsg_weedfarmer:client:removeWeedObject')
AddEventHandler('rsg_weedfarmer:client:removeWeedObject', function(plant)
...
RegisterNetEvent('rsg_weedfarmer:client:notify')
AddEventHandler('rsg_weedfarmer:client:notify', function(msg)
    QBCore.Functions.Notify(msg, "primary")
end)

RegisterNetEvent('rsg_weedfarmer:client:waterPlant')
AddEventHandler('rsg_weedfarmer:client:waterPlant', function()
	local hasItem = QBCore.Functions.HasItem("water_bottle")
	if hasItem then
		local entity = nil
		local plant = GetClosestPlant()
		local ped = GetPlayerPed(-1)
		...
		QBCore.Functions.Progressbar("watering_weed", "Watering...", 2000, false, false, {
			...
			FreezeEntityPosition(ped, false)
			TriggerServerEvent('rsg_weedfarmer:server:waterPlant', plant.id)
			ClearPedTasksImmediately(GetPlayerPed(-1))
			isDoingAction = false
		end)
	else
		QBCore.Functions.Notify('You have no water', 'error', 5000)
	end
end)

RegisterNetEvent('rsg_weedfarmer:client:feedPlant')
AddEventHandler('rsg_weedfarmer:client:feedPlant', function()
	local hasItem = QBCore.Functions.HasItem("weed_nutrition")
	if hasItem then
		local entity = nil
		local plant = GetClosestPlant()
		local ped = GetPlayerPed(-1)
		...
		QBCore.Functions.Progressbar("fertilizing_weed", "Fertilizing...", 2000, false, false, {
			...
			FreezeEntityPosition(ped, false)
			TriggerServerEvent('rsg_weedfarmer:server:feedPlant', plant.id)
			ClearPedTasksImmediately(GetPlayerPed(-1))
			isDoingAction = false
		end)
	else
		QBCore.Functions.Notify('You have no Fertilizer', 'error', 5000)
	end
end)

RegisterNetEvent('rsg_weedfarmer:client:updateWeedData')
AddEventHandler('rsg_weedfarmer:client:updateWeedData', function(data)
    Config.Plants = data
end)

RegisterNetEvent('rsg_weedfarmer:client:plantNewSeed')
AddEventHandler('rsg_weedfarmer:client:plantNewSeed', function(type)
    ...
    if CanPlantSeedHere(pos) and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
        QBCore.Functions.Progressbar("planting_weed", "Planting...", math.random(1000, 2000), false, false, {
            ...
            TriggerServerEvent('rsg_weedfarmer:server:plantNewSeed', type, pos)
        end)
    else
...
RegisterNetEvent('rsg_weedfarmer:client:plantSeedConfirm')
AddEventHandler('rsg_weedfarmer:client:plantSeedConfirm', function()
